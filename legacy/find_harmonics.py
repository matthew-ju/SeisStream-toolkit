...
def define_arguments():
    ...
    ### adds
    #fmin_plot
    helptext = 'filter fl for waveform plot' + \
        '(i.e. 1.0)'
    parser.add_argument('--fmin_plot', help=helptext, type=float, default=1.0)
    #fmax_plot
    helptext = 'filter fh for waveform plot' + \
        '(i.e. 10.0)'
    parser.add_argument('--fmax_plot', help=helptext, type=float, default=10.0)
    ### adde
...
def main():
    ...
    for fnam_smgr in tqdm(files):
        ...
        #fmin = 1e-2 # def
        #fmin = 1e-4
        calc_spec(fnam_smgr, fmin=args.fmin, fmax=10,
                  vmin=args.vmin, vmax=args.vmax, winlen=args.winlen,
                  plot_highfreq=not(args.skip_hf), 
                  pick_harmonics=None, 
                  pick_peak=None, 
                  fmin_pick=args.fmin, fmax_pick=args.fmax,
                  fmin_plot=args.fmin_plot, fmax_plot=args.fmax_plot,
                  s_threshold=-160, nharms=args.nharms, dpi=300,
                  path_out=args.out_path)
        ### adde
